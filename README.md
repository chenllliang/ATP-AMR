...
## Brief Introduction
SOTA AMR Parsing models using only 40k extra data. **Rank 1st** model on Structrual-Related Scores (SRL and Reentrancy).
...
Build Envrionment for BLINK, Note that BLINK has some requirements conflicts with spring, while the blinking script relies on both repo, so will build it upon spring
```bash
